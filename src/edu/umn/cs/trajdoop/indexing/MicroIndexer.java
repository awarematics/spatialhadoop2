 ...
 *
 *************************************************************************/
package edu.umn.cs.trajdoop.indexing;
 ...
 *
 */
public class MicroIndexer
{
	private static final Log LOG = LogFactory.getLog(MicroIndexer.class);
	...
	public static class PartitionerReduce<S extends Shape> extends
			...
			throws IOException, InterruptedException, ClassNotFoundException
	{
		...
		job.setJarByClass(MicroIndexer.class);
	...
	public static Partitioner createPartitioner(Path in, Path out, Configuration job,
	 ...
	 * @throws IOException
	 */
	public static Partitioner createPartitioner(Path[] ins, Path out, Configuration job,
			String partitionerName) throws IOException
	{
		try
		{
			Partitioner partitioner = null;
			Class<? extends Partitioner> partitionerClass = PartitionerClasses.get(partitionerName
					.toLowerCase());
			...
			long estimatedOutSize = (long) (inSize * (1.0 + job.getFloat(
					SpatialSite.INDEXING_OVERHEAD, 0.1f)));
			...
			int numPartitions = Math.max(1,
					(int) Math.ceil((float) estimatedOutSize / outBlockSize));
			LOG.info("Partitioning the space into " + numPartitions
					+ " partitions with capacity of " + partitionCapacity);
			...
			throws IOException, InterruptedException
	{
		...
		if (inFStatus != null && !inFStatus.isDir())
		{
			...
			// This is useful if the input is a hidden file which is
			// automatically
			// skipped by FileInputFormat. We need to plot a hidden file for the
			// case
		...
		final IndexRecordWriter<Shape> recordWriter = new IndexRecordWriter<Shape>(partitioner,
				replicate, sindex, outPath, conf);
		for (FileSplit fsplit : fsplits)
		{
			RecordReader<Rectangle, Iterable<Shape>> reader = inputFormat.createRecordReader(
					fsplit, null);
	...
	public static Job index(Path inPath, Path outPath, OperationsParams params) throws IOException,
	...
	protected static void printUsage()
	{
		...
		System.out
				.println("shape:<point|rectangle|polygon> - (*) Type of shapes stored in input file");
		System.out
				.println("sindex:<index> - (*) Type of spatial index (grid|str|str+|quadtree|zcurve|kdtree)");
		System.out.println("-overwrite - Overwrite output file without noitce");
		GenericOptionsParser.printGenericCommandUsage(System.out);
	}

	/*********************************************************
	 * New Codes for Micro-Index Support
	 *********************************************************/
	...
	public static Partitioner planMicroIndex(Path[] ins, Path out, Configuration job,
			String partitionerName, boolean isPrimaryIndex ) throws IOException, InstantiationException,
			IllegalAccessException
	{

		Partitioner partitioner = null;
		long estimatedOutSize = 0;
		long outBlockSize = 0;
		Vector<Point> samples = null;
		final Rectangle inMBR = (Rectangle) OperationsParams.getShape(job, "mbr");

		// Get the Partitioner Instance
		partitioner = getPartitionerInstance(job, partitionerName);
		
		// Get the sample data for approximated estimation
		samples = getSamples(job, ins, 0.0f, 0); // 0.0f, 0 mean default values

		if ( isPrimaryIndex == true ) // If Primary Index
		{
			long totalInSize = getTotalSize( job, ins );
			estimatedOutSize = getIndexOutSize(job, ins);
			outBlockSize = getOutBlockSize(job, out);
		} 
		else { // If Secondary Index
			
		}

		int partitionCapacity = (int) Math.max(1,
				Math.floor((double) samples.size() * outBlockSize / estimatedOutSize));
		...
		partitioner.createFromPoints(inMBR, samples.toArray(new Point[samples.size()]),
				partitionCapacity);
	...
	private static long getTotalSize(Configuration job, Path[] ins) throws IOException
	{
		long inSize = 0;
	...
	private static long getIndexOutSize(Configuration job, Path[] ins )
			throws IOException
	{
		long inSize = 0;
		long estimatedOutSize = 0;
		...
		estimatedOutSize = (long) (inSize * (1.0 + job
				.getFloat(SpatialSite.INDEXING_OVERHEAD, 0.1f)));

		return estimatedOutSize;
	}

	private static long getOutBlockSize(Configuration job, Path out) throws IOException
	{
		FileSystem outFS = out.getFileSystem(job);
		long outBlockSize = outFS.getDefaultBlockSize(out);
	...
	private static Vector<Point> getSamples(Configuration job, Path[] ins, float sample_ratio,
			long sample_size) throws IOException
	{

		final Vector<Point> sample = new Vector<Point>();
		sample_ratio = (sample_ratio > 0) ? 
							sample_ratio : job.getFloat(SpatialSite.SAMPLE_RATIO,0.01f);
		sample_size = (sample_size > 0) ? 
							sample_size : job.getLong(SpatialSite.SAMPLE_SIZE, 100 * 1024 * 1024);
	...
	private static Partitioner getPartitionerInstance(Configuration job, String partitionerName)
